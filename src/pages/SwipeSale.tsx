...
export default function SwipeSale() {
  ...
  const [autoFillSwipeType, setAutoFillSwipeType] = useState<string>(""); // Actually Swipe Type (Card/UPI)
  ...
  // Swipe Mode logic - if machine list available, just use that as mode
  // The screenshot shows "Swipe Type" -> "Values" and "Swipe Mode" -> "Values"
  // Assuming Swipe Type is Generic (Card/UPI) and Mode is specific machine name or vice-versa?
  // Let's assume AutoFill Swipe Type sets a default category, but user selects actual machine in Swipe Mode
  ...
  const createSwipeTransaction = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        swipe_type: autoFillSwipeType || "Card", // Default or from autofill
    ...
    onSuccess: () => {
      ...
      // Reset row fields but keep auto-fill
