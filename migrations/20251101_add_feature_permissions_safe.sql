-- Safe migration: Only creates if doesn't exist, and inserts only missing features

-- Create feature_permissions table if it doesn't exist
...
CREATE UNIQUE INDEX IF NOT EXISTS "feature_permissions_feature_key_idx" ON "feature_permissions" USING btree ("feature_key");

-- Create user_feature_access table if it doesn't exist
...
-- Add foreign key constraints if they don't exist (skip if already exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'user_feature_access_user_id_users_id_fk'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'user_feature_access_feature_id_feature_permissions_id_fk'
    ) THEN
...
CREATE UNIQUE INDEX IF NOT EXISTS "user_feature_access_user_feature_idx" ON "user_feature_access" USING btree ("user_id","feature_id");
