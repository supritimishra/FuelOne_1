...
async function run() {
    ...
    try {
        ...
        console.log(`Checking ${tenants.rows.length} tenants...`);
        ...
        for (const t of tenants.rows) {
            ...
            try {
                const res = await tClient.query(`
                  ...
                  FROM fuel_products 
                  WHERE product_name ILIKE '%raaaaaaaa3%' OR product_name ILIKE '%Diesel%'
              `);

                if (res.rows.length > 0) {
                    console.log(`Tenant: ${t.organization_name}`);
                    res.rows.forEach(r => console.log(`  - ${r.product_name} (${r.short_name})`));
                }
            } catch (e) {
